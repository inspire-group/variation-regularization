...
def get_args():
    ...
    parser.add_argument('--norm-source', default='l_inf', type=str, choices=['l_inf', 'l_2', 'stadv'])
    parser.add_argument('--norm-target', default='l_inf', type=str, choices=['l_inf', 'l_2', 'stadv'])
...
def main():
    ...
    if args.randomize:
        ...
        for i in range(args.randomize):
            ...
            if args.norm_source == 'stadv':
                stadv_var_source = StAdvVariation(nn.Sequential(Normalize(cifar10_mean, cifar10_std), model), num_iterations=100, bound=epsilon_source)
            if args.norm_target == 'stadv':
                stadv_var_target = StAdvVariation(nn.Sequential(Normalize(cifar10_mean, cifar10_std), model), num_iterations=100, bound=epsilon_target)
            model.eval()
            total_variation = 0
            total_variation_target = 0
            for i, batch in enumerate(test_batches):
                X, y = batch['input'], batch['target']
                if args.norm_source in ['l_inf', 'l_2']:
                    ...
                    Xvar1, Xvar2 = stadv_var_source(X, y)
                ...
                if args.norm_target in ['l_inf', 'l_2']:
                    ...
                    Xvar1, Xvar2 = stadv_var_target(X, y)
            ...
            for f in os.listdir(path):
                if 'model' in f or 'ckpt' in f:
                    ...
                    for i, batch in enumerate(test_batches):
                        ...
                        #else:
                        #    Xvar1, Xvar2 = stadv_var_source(X, y)
                        #var_source = torch.norm(model(normalize(Xvar1)) - model(normalize(Xvar2)), dim=-1, p=2).sum().item()
                        #total_variation += var_source
                        if args.norm_target in ['l_inf', 'l_2']:
                            Xvar1, Xvar2 = variation_single(X, model, epsilon_target, norm=args.norm_target, step_size=pgd_alpha, num_steps=args.attack_iters, normalize=normalize, logits=args.logits_var)
                        else:
                            Xvar1, Xvar2 = stadv_var_target(X, y)
                        var_target = torch.norm(model(normalize(Xvar1)) - model(normalize(Xvar2)), dim=-1, p=2).sum().item() 
                        total_variation_target += var_target
                        #print(var_source, var_target)
                    total_variation /= len(test_set)
                    total_variation_target /= len(test_set)
