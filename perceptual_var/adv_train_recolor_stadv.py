...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack', type=str, nargs='+',
                        help='attack(s) to harden against')
    parser.add_argument('--stadv', action='store_true')
    ...
    is_stadv = args.stadv # otherwise we will assume it is recolor
    if is_stadv:
        args.attack = ['StAdvAttack(model, num_iterations=10, bound=0.03)']
        var = eval(args.attack[0].replace('StAdvAttack', 'StAdvVariation'))
    else:
        args.attack = ['ReColorAdvAttack(model, num_iterations=10, bound=0.04)']
        var = eval(args.attack[0].replace('ReColorAdvAttack', 'ReColorAdvVariation'))
    ...
    validation_attacks = validation_attacks + attacks
    ...
    def run_iter(
        ...
        var_input1, var_input2 = var(inputs[to_attack], labels[to_attack])
        ...
        if args.var_reg > 0:
            logits_var1 = model(var_input1)
            logits_var2 = model(var_input2)
