...
from .models import AlexNet, AlexNetFeatureModel, CifarAlexNet, FeatureModel
...
def get_lpips_model(
    ...
    if lpips_model_spec == 'self':
        ...
        if _cached_alexnet is None:
            ...
            state_dict = torch.load('data/checkpoints/alexnet-owt-7be5be79.pth')
...
class FastLagrangePerceptualVariation(nn.Module):
    def __init__(self, model, bound=0.5, step=None, num_iterations=20,
                 lam=10, h=1e-1, lpips_model='self', decay_step_size=True,
                 increase_lambda=True, projection='none', kappa=math.inf,
                 include_image_as_activation=False, randomize=False):
    ...
    def forward(self, inputs, labels):
        ...
        for attack_iter in range(self.num_iterations):
            ...
            if self.model == self.lpips_model:
                adv_features, adv_logits = \
                    self._get_features_logits(adv_inputs)
                adv_features2, adv_logits2 = \
                    self._get_features_logits(adv_inputs2)
            else:
                adv_features = self._get_features(adv_inputs)
                adv_logits = self.model(adv_inputs)
                adv_features2 = self._get_features(adv_inputs2)
                adv_logits2 = self.model(adv_inputs2)


            adv_loss = self.loss(adv_logits, adv_logits2)

            lpips_dists = (adv_features - input_features).norm(dim=1)
            lpips_dists2 = (adv_features2 - input_features).norm(dim=1)

            loss = -adv_loss + lam * F.relu(lpips_dists - self.bound) + lam * F.relu(lpips_dists2 - self.bound)
            loss.sum().backward()
        ...
        return (self.projection(inputs, adv_inputs, input_features).detach(), 
               self.projection(inputs, adv_inputs2, input_features).detach())
