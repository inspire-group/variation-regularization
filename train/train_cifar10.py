...
from wide_resnet import wrn_28_10
...
def get_args():
    ...
    parser.add_argument('--fix-act', action='store_true')
    parser.add_argument('--fix-act-val', default=0.0, type=float)
    parser.add_argument('--act-beta-l2', default=5.0, type=float)
    parser.add_argument('--act-beta-clip', default=0.1, type=float)
    parser.add_argument('--batch-size', default=256, type=int)
    parser.add_argument('--batch-size-test', default=256, type=int)
...
def main():
    ...
    if args.model == 'resnet18':
        ...
        model = resnet50(num_classes=10, activation=act)
        ...
        for i, batch in enumerate(train_batches):
            ...
            with torch.no_grad():
                if args.var_reg > 0:
                    if args.var_reg_type == 'var':
                        Xvar1, Xvar2 = variation_single(X, model, epsilon, norm=args.norm, step_size=pgd_alpha, num_steps=args.attack_iters, normalize=normalize, logits=args.logits_reg, var_norm=args.var_norm)
                    else:
                        Xvar1 = upper_var_single(X, model, epsilon, norm=args.norm, step_size=pgd_alpha, num_steps=args.attack_iters, normalize=normalize, logits=args.logits_reg, var_norm=args.var_norm)
                        Xvar2 = X 
            ...
            if args.var_reg > 0:
                if not args.logits_reg:
                    var_reg_loss = torch.norm(model(Xvar1, feature=True) - model(Xvar2, feature=True), float(args.var_norm.split('_')[1]), dim=-1).sum() / len(X)
                else:
                    var_reg_loss = torch.norm(model(Xvar1) - model(Xvar2), float(args.var_norm.split('_')[1]), dim=-1).sum() / len(X)
