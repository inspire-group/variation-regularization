...
def get_args():
    ...
    parser.add_argument('--data-dir', default='/scratch/gpfs/sihuid/data/cifar100', type=str)
...
def main():
    ...
    if args.model == 'resnet18':
        model = resnet18(num_classes=100, activation=act)
    elif args.model == 'wrn_28_10':
        model = wrn_28_10(num_classes=100, activation=act)
    elif args.model == 'vgg16':
        model = vgg16_bn(num_classes=100, activation=act)
    else:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if args.var_reg > 0:
                var_reg_loss = torch.norm(model(Xvar1, feature=(not args.logits_reg)) - model(Xvar2, feature=(not args.logits_reg)), float(args.var_norm.split('_')[1]), dim=-1).mean()
