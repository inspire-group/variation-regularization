...
from train.utils_var import variation_single
...
if __name__ == '__main__':
    ...
    parser.add_argument('--norm_loss', type=str, default='Linf', choices=['L2', 'Linf'])
    parser.add_argument('--norm_var', type=str, default='Linf', choices=['L2', 'Linf'])
    parser.add_argument('--epsilon_loss', type=float, default=16./255.)
    parser.add_argument('--epsilon_var', type=float, default=8./255.)
    ...
    # evaluate
    adv = APGDAttack(model, norm=args.norm_loss, eps=args.epsilon_loss)
    if args.norm_loss != args.norm_var:
        adv_source = APGDAttack(model, norm=args.norm_var, eps=args.epsilon_var)
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            if args.norm_loss != args.norm_var:
                # take the union
                x_adv2 = adv_source.perturb(x, y)
                pred2 = model(x_adv2)
                loss += torch.maximum(criterion(pred, y), criterion(pred2, y)).sum()
            else:
                loss += criterion(pred, y)
            if args.norm_var == 'Linf':
                norm = 'l_inf'
            else:
                norm = 'l_2'
            x1, x2 = variation_single(x, model, args.epsilon_var, norm, step_size=args.alpha, num_steps=10, logits=True, var_norm='l_2')
            var += torch.norm(model(x1)- model(x2), 2, dim=-1).sum()
        print('ce loss', loss / len(test_loader.dataset))
        print('var', var / len(test_loader.dataset))